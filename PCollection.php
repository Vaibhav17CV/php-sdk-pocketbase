...
class Collection
{
    ...
    public function getList(int $start = 1, int $end = 50, array $queryParams = []): array
    {
        ...
        $response = $this->makeRequest($this->url . "/api/collections/" . $this->collection . "/records?" . $getParams, 'GET');
    ...
    {
        $result = $this->makeRequest($this->url . "/api/collections/users/auth-with-password", 'POST', ['identity' => $email, 'password' => $password]);
    ...
    {
        return $this->makeRequest($this->url . "/api/collections/" . $this->collection . "/records", 'POST', json_encode($bodyParams));
    }
    ...
    public function update(string $recordId, array $bodyParams = [], array $queryParams = []): void
    {
        ...
        $this->makeRequest($this->url . "/api/collections/" . $this->collection . "/records/" . $recordId, 'PATCH', json_encode($bodyParams));
    }
    ...
    {
        $this->makeRequest($this->url . "/api/collections/" . $this->collection . "/records/" . $recordId, 'DELETE');
    }
     ...
     * @return bool|string
     */
    public function makeRequest(string $url, string $method, $bodyParams = []): string
    {
        $ch = curl_init();
    ...
    {
        $output = $this->makeRequest($this->url . "/api/collections/" . $this->collection . "/records/" . $recordId, 'GET');
    ...
    public function authAsAdmin(string $email, string $password): void
    {
        ...
        $output = $this->makeRequest($this->url . "/api/admins/auth-with-password", 'POST', $bodyParams);
